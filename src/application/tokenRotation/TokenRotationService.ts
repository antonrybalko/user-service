...
export class TokenRotationService {
  ...
  async rotateTokens(refreshTokenString: string): Promise<TokenPair> {
    try {
      ...
      const accessTokenExpiresAt = new Date(
        now.getTime() + tokenPair.accessTokenExpiresIn * 1000,
      );
      const refreshTokenExpiresAt = new Date(
        now.getTime() + tokenPair.refreshTokenExpiresIn * 1000,
      );
  ...
  async revokeAllUserTokens(userGuid: string): Promise<number> {
    try {
      ...
      throw new Error(
        `Failed to delete user tokens: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async cleanupExpiredTokens(): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to clean up expired tokens: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
}
