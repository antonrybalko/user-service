...
export class RegistrationRepository {
    ...
    async checkIfEmailOrPhoneExists({email, phoneNumber}: RegistrationDto): Promise<boolean> {
        // Check for existing user
        const emailOrPhoneExists = await this.userRepository.findOne({ where: [{ email }, { phoneNumber }] });
